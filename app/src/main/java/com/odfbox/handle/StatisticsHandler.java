...
public class StatisticsHandler extends BaseResponseHandler {

    public StatisticsHandler() {
        this(DEFAULT_CHARSET);
    }

    public StatisticsHandler(String encoding) {
        super(encoding);
    }

    public void onSuccess(Statistics odfbox) {
            ...
            public void run() {
                postRunnable(new Runnable() {
                    ...
                    public void run() {
                        try {
                            if (statusCode == 200) {
                                onSuccess(JSON.parseObject(response.toString(), Statistics.class));
                                ;
                            } else {
